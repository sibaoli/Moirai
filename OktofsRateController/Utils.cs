...
namespace OktofsRateControllerNamespace
{
    ...
    /// <summary>
    /// Generic workitem queue.  This class has many bugs.  I am only
    /// fixing the most critical until I discover whether it really
    /// needs to exist at all or not.
    /// </summary>
    /// <typeparam name="T">Type of items we queue.</typeparam>
    public class WorkQueue<T>
    {
        /// <summary>
        /// The WorkQueue will call a function of this type on each work item
        /// </summary>
        /// <param name="workitem">Workitem to operate on</param>
        public delegate void Callback(T workitem);

        private Callback callback;  // function we call to process an item

        // How many packets can we hold?
        private int maxQueueDepth;
        private int maxReaders;

        private object qLock = new object();
        private T[] workitemQueue;
        private int readPtr;
        private int writePtr;
        private int numReaders;
        private int numQueuedPackets;
        private WaitCallback readerLoopCallback;
        ...
        /// <summary>
        /// Create and configure a new WorkQueue.
        /// </summary>
        /// <param name="svc">Service we are in, so we can QueueUserWorkItems to process items.</param>
        /// <param name="callback">Function to call on each queued item.</param>
        /// <param name="maxReaders">Maximum number of workitems we will process in parallel.</param>
        /// <param name="maxQueueDepth">Maximum number of items we will queue.</param>
        public WorkQueue(Callback callback, int maxReaders, int maxQueueDepth)
        {
            this.callback = callback;
            this.readerLoopCallback = new WaitCallback(this.readerLoop);
            this.maxReaders = maxReaders;
            this.maxQueueDepth = maxQueueDepth;
            this.workitemQueue = new T[maxQueueDepth];

#if !USE_WORK_ITEMS
            for (int i = 0; i < this.maxReaders; i++)
            {
                var thd = new Thread(this.threadEntry);
                thd.IsBackground = true;
                thd.Name = this.GetType().FullName;
                thd.Start();
            }
#endif
        }
        ...
        /// <summary>
        /// Add workitem to the queue.
        /// </summary>
        /// <param name="workitem">Item to be added</param>
        /// <returns>True if successfull, or false if workitem was dropped because queue was full</returns>
        public bool Enqueue(T workitem)
        {
            lock (this.qLock)
            {
                if (this.numQueuedPackets >= this.maxQueueDepth)
                {
                    this.stat.Drops++;
                    return false; // full: dropped
                }

                this.numQueuedPackets++;

                this.stat.TotQlen += this.numQueuedPackets;
                this.stat.TotQops++;

                this.workitemQueue[this.writePtr] = workitem;
                this.writePtr = (this.writePtr + 1) % this.maxQueueDepth;


#if USE_WORK_ITEMS
		if (this.numReaders < this.maxReaders)
		{
		    this.svc.node.QueueUserWorkItem(this.readerLoopCallback);
		    Trace.Assert(this.numReaders >= 0 && this.numReaders < this.maxReaders);
		    this.numReaders++;
		    this.stat.Wakes++;
		}
#else
                if (this.numReaders < this.numQueuedPackets &&
                    this.numReaders < this.maxReaders)
                    Monitor.Pulse(this.qLock);
#endif
        ...
        private void readerLoop(object state)
        {
            ...
            while (true)
            {
                lock (this.qLock)
                {
                    if (this.numQueuedPackets == 0)
                    {
                        Debug.Assert(this.numReaders > 0 && this.numReaders <= this.maxReaders);
                        this.numReaders--;
                        this.stat.Yields++;
                        return; // nothing left to do, exit
                    }

                    workitem = this.workitemQueue[this.readPtr];
                    this.readPtr = (this.readPtr + 1) % this.maxQueueDepth;
                    this.numQueuedPackets--;
        ...
        private void threadEntry()
        {
            while (true)
            {
                lock (this.qLock)
                {
                    // docs for Wait() do not say it is subject to spurious wakeups.
                    while (this.numQueuedPackets == 0)
                        Monitor.Wait(this.qLock);
                    Debug.Assert(this.numReaders >= 0 && this.numReaders < this.maxReaders);
                    this.numReaders++;
                    this.stat.Wakes++;
                }
        ...
        /// <summary>
        /// Print and zero the summary stats
        /// </summary>
        public void DumpCounters()
        {
            ...
            if (st.Drops != 0)
                Debug.WriteLine(String.Format("RxQ dropped {0}", st.Drops));
        }
    }
